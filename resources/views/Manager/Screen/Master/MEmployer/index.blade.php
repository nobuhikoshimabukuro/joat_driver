  ...
  <table class="table table-hover table-fixed-header table-nowrap mb-0">
    <thead>
      <tr>
        ...
        <th class="w-150px {{$TextClass[$TextClassIndex++]}}">求人元ID</th>
        <th class="{{$TextClass[$TextClassIndex++]}}">求人元名</th>
    ...
    <tbody>
      @foreach ($m_employer as $item)
        ...
        @endphp
        <tr data-target_row="{{$item->employer_id}}" class="@if($DeleteFlg) delete_row  @endif">
          <td class="{{$TextClass[$TextClassIndex++]}}">
            <button class="btn btn-outline-primary page-transition-button mb-2"
              data-url="{{route('manager.master.m_employer.entry')}}?employer_id={{$item->employer_id }}"
              data-process="1">編集</button>
          </td>
          <td class="{{$TextClass[$TextClassIndex++]}}">{{ $item->employer_id }}</td>
          <td class="{{$TextClass[$TextClassIndex++]}}">
            <ruby>
              {{ $item->employer_name }}

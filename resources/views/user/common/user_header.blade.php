    ...
    <nav role="navigation">
      <div id="menuToggle">
        ...
        <ul id="menu">            
            ...
            @foreach (collect(session("user_menu_info"))->sortBy('sort_order') as $info)
            ...
            @endforeach
{{-- 
            @foreach (session("user_menu_info") as $info)
                @if($info->header_flg == 1)
                    <li>
                        <label>
                            <a href="{{$info->href}}">{{$info->title}}</a>                    
...
<script>
  $(document).on('click', function(e) {
    ...
    // メニューが開いていて、クリックしたのがメニューToggle内じゃない場合
    if ($checkbox.prop('checked') && !$menuToggle.is(e.target) && $menuToggle.has(e.target).length === 0) {
